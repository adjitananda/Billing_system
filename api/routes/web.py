...
async def client_detail(request: Request, client_id: int):
    ...
    # Расчет дневной стоимости для каждого сервера и общей суммы
    ...
    try:
        ...
        active_servers = get_active_servers_on_date(conn, today)
        # Создаем словарь активных серверов для быстрой проверки
        active_servers_dict = {s["id"]: s for s in active_servers}
        
        # Получаем цены на сегодня
        prices = get_prices_on_date(conn, today)
        
        # Для каждого сервера из списка рассчитываем стоимость
        for server in servers:
            server_id = server.get("id")
            daily_cost = 0.0
            
            # Проверяем, активен ли сервер на сегодня
            if server_id in active_servers_dict and prices:
                # Получаем конфигурацию на сегодня
                config = get_config_on_date(conn, server_id, today)
                if config:
                    cost_dict = calculate_server_cost(config, prices)
                    daily_cost = float(cost_dict["total_cost"])
            
            # Добавляем поле daily_cost к серверу
            server["daily_cost"] = daily_cost
    ...
    monthly_cost_approx = daily_cost_total * 31
...
async def activate_server_redirect(request: Request, server_id: int):
    ...
    async with httpx.AsyncClient() as client:
        await client.post(f"{API_BASE_URL}/servers/{server_id}/activate")
    return RedirectResponse(url=f"/servers/{server_id}", status_code=303)
